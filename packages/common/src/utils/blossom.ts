...
import { BlobDescriptor, EventTemplate, SignedEvent, Signer } from "blossom-client-sdk";

const blossomUrlRegex = /\/[0-9a-f]{64}(\.\w+)?$/;

export function signWith(signer: NDKSigner, ndk: NDK): Signer {
    return (draft: EventTemplate) => sign(draft, signer, ndk) as Promise<SignedEvent>;
}

export async function sign(draft: EventTemplate, signer: NDKSigner | undefined, ndk: NDK) {
    const e = new NDKEvent(ndk, draft as any);
    await e.sign(signer);
    return e.toNostrEvent();
}

export function generateMediaEventFromBlobDescriptor(blob: BlobDescriptor, ndk: NDK) {
...
async function getBlossomListFor(user: NDKUser, ndk: NDK) {
...
export function createBlossom({user, ndk}: { user: NDKUser, ndk: NDK }) {
