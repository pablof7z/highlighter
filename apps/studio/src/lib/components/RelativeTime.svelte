<script lang="ts">
	...
	export let timestamp: number | undefined =
		(published_at ? parseInt(published_at) : event?.created_at)! * 1000;
	...
	const updateDiff = () => {
		distanceOfTime = timestamp ? Math.floor((Date.now() - timestamp) / 1000) : 99999999;
	};
...
{#if !Number.isNaN(timestamp)}
	{#if distanceOfTime < 60}
		<span class={$$props.class || ``}> just now </span>
	...
	{:else}
		<Time live={true} {timestamp} class={$$props.class || ``} format="M/D/YY" />
	{/if}
{/if}
