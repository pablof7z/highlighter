...
{#if choosenAvatars.length >= 1}
    {#key choosenAvatars[0]}
        <Avatar size="small" class="!w-8 !h-8 m-[2px]" type="circle" pubkey={choosenAvatars[0]} {subOpts} />
    {/key}

    {#if choosenAvatars.length >= 2}
        <div class="flex items-start">
            {#key choosenAvatars[1]}
                <Avatar size="small" class="m-[2px] !w-6 !h-6" type="circle" pubkey={choosenAvatars[1]} {subOpts} />
            {/key}
            {#if choosenAvatars.length >= 3}
                {#key choosenAvatars[2]}
                    <Avatar size="small" class="m-[2px] !w-4 !h-4" type="circle" pubkey={choosenAvatars[2]} {subOpts} />
                {/key}
            {/if}
        </div>
    {/if}
