...
<div class="
    ...
    flex flex-col
    {collapsed ? "" : "max-sm:!fixed max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:top-0 max-sm:!z-[50] max-sm:!bg-base-100/90 max-sm:backdrop-blur-[50px] max-sm:overflow-y-auto max-sm:h-[100dvh]"}
" class:hidden={publishing}>
    {#key resetEditorAt}
        <ContentEditor
            ...
            allowMarkdown={false}
            class="
                {collapsed ? "sm:min-h-[7rem]" : "sm:min-h-[15rem] flex-grow"}
            {$$props.class??""}"
        />
    ...
    <div class="absolute -z-1 bottom-4 left-0 flex flex-row gap-6 pt-2 sm:border-t border-base-300 w-full px-4 justify-end
        {collapsed ? "" : "max-sm:justify-between"}
    ">
        <button class="
            button
