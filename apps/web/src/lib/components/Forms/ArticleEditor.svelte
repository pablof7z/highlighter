            ...
            <div class="flex flex-row items-center gap-4 my-2">
                {#if !article.image || $appMobileView}
                    <div class="flex flex-row items-center gap-[1px]">
                        <Button variant="default" class="rounded-r-none" size="xs" on:click={() => openModal(CoverImageModal, { article, onSave: onSaveCover })}>
                            {article.image ? "Change" : "Add"} cover image
                        </Button>
                    
                        <Button variant="default" size="xs" class="rounded-l-none" on:click={fetchRandomImage}>
                ...
                {#if !showSummary}
                    <Button size="xs" variant="outline" on:click={() => showSummary = !showSummary}>Add summary</Button>
                {/if}
                {#if !showTags}
                    <Button size="xs" variant="outline" on:click={() => openModal(TagInputModal, { event: article, onSave: onSaveTags })}>Add #hashtags</Button>
                {/if}
            </div>
        {/if}
        {#if showTags}
