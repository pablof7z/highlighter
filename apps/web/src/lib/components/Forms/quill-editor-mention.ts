...
import { Hexpubkey, NDKEvent, NDKRelaySet, NDKUser, NDKUserProfile, profileFromEvent } from '@nostr-dev-kit/ndk';
import { get } from 'svelte/store';

export default async function(searchTerm, renderList) {
    const $ndk = get(ndk);
    const haveCache = !!$ndk.cacheAdapter?.getProfiles;
    const result: { id: string, pubkey?: string, avatar?: string, value?: string, nip05?: string, followed: boolean }[] = [];
