<script lang="ts">
	...
	if (target instanceof NDKEvent) {
		zapSplits = target.getMatchingTags('zap').map((zapTag: NDKTag) => {
			return [zapTag[1], parseInt(zapTag[3] ?? '1'), 0];
		});
	} else if (target instanceof NDKUser) {
		zapSplits = [[target.pubkey, 1, 0]];
	} else {
...
<div class="flex flex-row flex-nowrap justify-center gap-4 max-lg:flex-col">
	{#if zapSent}
	...
	{:else}
		<div class="m-2 flex flex-col gap-8">
			{#if !skipSplits}
			...
			{#if prs.length > 0}
				<div class="no-scrollbar flex flex-row flex-nowrap gap-2 overflow-x-auto">
					{#each prs as pr}
				...
				<div class="flex flex-col gap-3">
					...
					<div class="flex flex-row gap-4 max-lg:flex-col">
						<div class="flex flex-row gap-3">
							<ZapAmountButton title={'1K'} bind:group={amount} value={1000}>
								<HeartStraight class="h-full w-full" />
							</ZapAmountButton>
					...
					<div class="flex w-full flex-col items-center justify-center gap-4 pt-4">
						{#if isCustomAmountSelected}
							...
							{/if}
						{/if}
						<Input class="w-full text-lg" placeholder="Add a comment..." bind:value={comment} />
					</div>
				</div>
