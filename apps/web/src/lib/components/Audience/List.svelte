...
<div class="flex flex-col gap-2">
	<RadioButton bind:currentValue={$state.scope} value="public" color="default">
		<span class:text-muted-foreground={$state.scope !== 'public'}> Everyone </span>
		<span slot="description"> Anyone can see, share and interact with this content. </span>
	...
	<RadioButton bind:currentValue={$state.scope} value="private" color="default">
		...
		<span slot="description"> Only paid subscribers will be able to see this content. </span>
		<svelte:fragment slot="icon">
			<Lock />
		</svelte:fragment>
	...
	{#if $tierList && $tierList.items.length > 0 && $groupsList && $groupsList.items.length > 0}
		<div class="bg-secondary/30 mt-4 flex w-full flex-col rounded border p-2 px-4 text-sm">
