...
<ModalShell color="glassy" class="max-h-[80vh] overflow-y-auto">
    <div class="flex flex-col gap-1 items-center w-full">
        <div class="font-semibold text-foreground text-3xl">Teaser Preview</div>
    ...
    <div class="flex flex-col items-stretch gap-2 w-full">
        <Textarea
            ...
            autofocus={true}
            class="w-full sm:rounded-xl max-sm:border-none flex-grow font-normal text-lg leading-normal !bg-white/5 !border-border text-neutral-400 p-6"
    ...
    {#if content.length > 1}
        <div class="w-full bg-foreground/10 p-4 rounded scale-90 flex flex-col gap-6">
            <div class="article">
                {#key content}
            ...
            <UserProfile user={$user} let:fetching let:userProfile>
                <div class="flex flex-row gap-4 items-center">
                    ...
                    <Name {userProfile} {fetching} class="text-foreground text-[15px] font-semibold" />
                </div>
            </UserProfile>
        </div>
    {/if}
    ...
    <div class="flex flex-row gap-10 items-center justify-end w-full">
        <button class="text-foreground text-base" on:click={closeModal}>
            Close
        </button>
